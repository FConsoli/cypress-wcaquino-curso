...
describe('Helpers', () => {
    it('Wrap', () => {
        ...
        cy.get('#buttonSimple')
            .then(() => console.log('Encontrei o botão'))
        cy.wrap(promise)
    ...
    it('Its', () => {
        ...
        cy.wrap(obj)
            .its('nome').should('be.equal', 'User')

        const obj2 = { nome: 'User', idade: 20, endereco: { rua: 'dos bobos' } }
        cy.wrap(obj2)
            .its('endereco').should('have.property', 'rua')
        /*cy.wrap(obj2)
            .its('endereco').its('rua').should('contain', 'bobos')*/
        cy.wrap(obj2)
            .its('endereco.rua').should('contain', 'bobos')
